...
Le présent Projet de Fin d'Études s'inscrit dans cette problématique. Réalisé dans le cadre d'un stage chez \textbf{SEVENAPP}~\cite{sevenapp}, en mission de consultant Data auprès de \textbf{EQDOM}~\cite{eqdom}, il porte sur la conception et la mise en œuvre d'une \textbf{Cloud Data Platform} moderne, déployée sur \textbf{AWS EKS}~\cite{aws-eks}, fondée sur une architecture \textbf{Lakehouse}~\cite{lakehouse-paper} et structurée selon le modèle \textbf{médaillon}~\cite{databricks-medallion} : Bronze, Silver et Gold.

L'objectif du projet n'est pas seulement d'assembler des outils open source. Il s'agit de construire un socle reproductible, sécurisé et exploitable, capable de couvrir l'ensemble du cycle de vie de la donnée : ingestion batch et streaming, stockage objet, catalogage, transformations distribuées, orchestration, exposition SQL/BI, supervision et automatisation de l'infrastructure. Le projet accorde également une attention particulière aux contraintes opérationnelles : gestion des secrets, isolation Kubernetes~\cite{kubernetes}, optimisation des coûts, traçabilité des choix d'architecture et capacité à maintenir un environnement fiable et industrialisable.

La solution proposée repose sur un ensemble cohérent de technologies : Terraform~\cite{terraform} pour l'Infrastructure as Code, Cloudflare R2~\cite{cloudflare-r2} pour le backend d'état Terraform, Amazon EKS pour l'orchestration Kubernetes, MinIO~\cite{minio} pour le stockage S3-compatible, Apache Iceberg~\cite{apache-iceberg} et Hive Metastore~\cite{apache-hive} pour la couche Lakehouse, Kafka~\cite{apache-kafka}/Strimzi~\cite{strimzi} et Debezium~\cite{debezium} pour la capture de changements, Apache Spark~\cite{apache-spark} et dbt~\cite{dbt} pour les transformations, Apache Airflow~\cite{apache-airflow} pour l'orchestration, Dremio~\cite{dremio} pour la virtualisation SQL, puis Prometheus~\cite{prometheus}, Grafana~\cite{grafana}, Fluent Bit~\cite{fluent-bit} et OpenObserve~\cite{openobserve} pour l'observabilité.
